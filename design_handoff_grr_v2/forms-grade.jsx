...
function UnitFormSheet({ open, mode, unit, onClose, onSave }) {
  const blank = { name: '', address: window.PROPERTY.name, currentRent: '', sqft: 700, beds: 1, baths: 1, amenities: [] };
  const [f, setF] = useStateF(blank);

  useEffectF(() => {
    if (!open) return;
    if (mode === 'edit' && unit) {
      setF({ name: unit.name, address: unit.address || unit.building, currentRent: unit.currentRent, sqft: unit.sqft, beds: unit.beds, baths: unit.baths, amenities: [...(unit.amenities || [])] });
    } else {
      setF(blank);
    }
  ...
  return (
    <window.ModalSheet open={open} onClose={onClose} wide label={mode === 'edit' ? 'Edit unit' : 'Add unit'}>
      <div className="ur-form">
        ...
        <label className="ur-field">
          ...
          <div className="ur-input__money">
            ...
            <input className="ur-input ur-input--money" type="number" inputMode="numeric" value={f.currentRent}
              placeholder="e.g. 1,800" onChange={(e) => set('currentRent', e.target.value === '' ? '' : parseInt(e.target.value || '0', 10))} />
          </div>
        </label>
