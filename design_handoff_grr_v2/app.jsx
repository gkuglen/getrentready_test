...
function UnitDetail({ unit, st, setVal, onOpenComp, onSave, dir }) {
  // live unit reflects the (draggable) current rent so every card recomputes
  const liveUnit = { ...unit, currentRent: st.currentRent };
  return (
    <div className="ur-stack" key={unit.id} data-dir={dir}>
      <div className="ur-loc">{unit.address || unit.building} · {unit.city}</div>
      <window.UnitHeader unit={liveUnit} target={st.target} onCurrent={(v) => setVal('currentRent', v)} />
      <window.OpportunityCard unit={liveUnit} target={st.target} />
      <window.MarketCard unit={liveUnit} target={st.target} />
      <window.BudgetCard unit={liveUnit} target={st.target} capRate={st.capRate} coc={st.coc}
        onCap={(v) => setVal('capRate', v)} onCoc={(v) => setVal('coc', v)} />
      <window.GradeLadderCard unit={liveUnit} target={st.target} />
      <window.CompsRail unit={liveUnit} onOpen={onOpenComp} />
...
function App() {
  ...
  const [state, setState] = useStateA(() => {
    const o = {};
    window.UNITS.forEach((u) => { o[u.id] = { target: u.suggestedTarget, currentRent: u.currentRent, capRate: u.capRate, coc: u.coc }; });
    return o;
  });
  ...
  const addUnit = (f) => {
    ...
    const currentRent = f.currentRent > 0 ? f.currentRent : Math.round((1150 + f.beds * 540 + f.sqft * 1.05) / 5) * 5;
    ...
    setUnits((arr) => [...arr, u]);
    setState((s) => ({ ...s, [u.id]: { target: u.suggestedTarget, currentRent: u.currentRent, capRate: u.capRate, coc: u.coc } }));
    setForm({ open: false, mode: 'add' });
    openUnit(u.id);
  };

  const saveEdit = (f) => {
    const u = window.makeUnit({ ...rawFromUnit(unit), n: f.name, address: f.address, beds: f.beds, baths: f.baths, sqft: f.sqft, currentRent: f.currentRent > 0 ? f.currentRent : unit.currentRent, amenities: f.amenities });
    setUnits((arr) => arr.map((x) => (x.id === u.id ? u : x)));
    setState((s) => ({ ...s, [u.id]: { ...s[u.id], currentRent: u.currentRent } }));
  ...
  return (
    <div className="ur-root" style={rootStyle} data-card={t.cardStyle}>
      <div className="ur-shell">
          ...
          <div className="ur-topbar__r">
            {screen === 'roll' ? (
              <label className="ur-search">
                <svg className="ur-search__icon" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="7" /><path d="M21 21l-4.3-4.3" /></svg>
                <input className="ur-search__input" type="search" placeholder="Locate a property" aria-label="Locate a property" />
              </label>
            ) : (
        ...
        {screen === 'roll'
          ? <window.RentRollScreen units={units} state={state} onOpenUnit={openUnit} onAdd={() => setForm({ open: true, mode: 'add' })} />
      ...
      <window.UnitFormSheet open={form.open} mode={form.mode} unit={{ ...unit, currentRent: st.currentRent }}
